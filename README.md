...
Livebook is a web application for writing interactive and collaborative code notebooks for Elixir, built with [Phoenix LiveView](https://github.com/phoenixframework/phoenix_live_view). It features:
  ...
  * Shareable: notebooks are stored in the `.livemd` format, which is a subset of Markdown with annotations, visualizations & diagrams with [Mermaid](https://mermaid-js.github.io/mermaid) and [KaTex](https://katex.org/) for mathematical formulas. note This means your notebooks can be saved, easily shared, and play well with version control.
