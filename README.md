...
`~/.livebookdesktop.sh` on macOS or `~/.livebookdesktop.bat` on Windows.
This file can set environment variables used by Livebook, such as [the
...
`ERL_AFLAGS="-proto_dist inet6_tcp"` if you need Livebook Desktop to run
over IPv6. Be careful when modifying said files, Livebook may be unable
to start if configured incorrectly.
