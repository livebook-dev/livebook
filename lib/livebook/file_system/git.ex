...
defimpl Livebook.FileSystem, for: Livebook.FileSystem.Git do
  ...
  def write(_file_system, _path, _content) do
  ...
  def create_dir(_file_system, _path) do
    {:error, "not implemented"}
  end
  ...
  def write_stream_init(_file_system, _path, _opts) do
    {:error, "not implemented"}
  end

  def write_stream_chunk(_file_system, _state, _chunk) do
    {:error, "not implemented"}
  end
  ...
  def read_stream_into(_file_system, _path, _collectable) do
    {:error, "not implemented"}
  end
  ...
  # Ensure this File System belongs only to Team hub
  def load(file_system, %{hub_id: "team-" <> _} = fields) do
    %{
      file_system
      | id: fields.id,
        repo_url: fields.repo_url,
  ...
  # Ensure this File System belongs only to Team hub
  def dump(%{hub_id: "team-" <> _} = file_system) do
    file_system
    |> Map.from_struct()
