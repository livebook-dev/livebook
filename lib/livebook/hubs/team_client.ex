defmodule Livebook.Hubs.TeamClient do
  ...
  def handle_call({:check_deployment_authorization, user_id, id}, _caller, state) do
    ...
    if state.deployment_group_id || user_id == nil do
      ...
      case fetch_deployment_group(id, state) do
        {:ok, deployment_group} ->
          authorized_user = %Teams.AuthorizedUser{
          ...
          authorized? =
            not deployment_group.deploy_auth or
              authorized_user in deployment_group.authorized_users
  ...
  defp build_deployment_group(state, %LivebookProto.DeploymentGroup{} = deployment_group) do
    ...
    authorized_users = build_authorized_users(deployment_group)
  ...
  defp build_deployment_group(state, deployment_group_updated) do
    ...
    authorized_users = build_authorized_users(deployment_group_updated)
  ...
  defp build_authorized_users(%{authorized_users: authorized_users}) do
    for authorized_user <- authorized_users do
      %Teams.AuthorizedUser{
        user_id: authorized_user.user_id,
        deployment_group_id: authorized_user.deployment_group_id
      }
    end
  end
    ...
    with {:ok, current_deployment_group} <- fetch_deployment_group(deployment_group.id, state) do
      cond do
        ...
        state.deployment_group_id == nil and
            (current_deployment_group.authorized_users !=
               deployment_group.authorized_users or
