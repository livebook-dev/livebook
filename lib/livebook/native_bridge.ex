defmodule Livebook.NativeBridge do
  @moduledoc """
  NativeBridge communicates with the native part of an iOS/Android application
  # Protocol
  ...
  def handle_call(
        ...
        _from,
        state = %NativeBridge{events: events, subscribers: subscribers}
      ) do
    for event <- events do
      send(pid, event)
    end

    {:reply, :ok, %NativeBridge{state | events: [], subscribers: [pid | subscribers]}}
  end

  def handle_call(
        {:bridge_call, ref, json},
        from,
        state = %NativeBridge{socket: socket, requests: reqs}
      ) do
    if socket do
      message = <<ref::unsigned-size(64), json::binary>>
      :gen_tcp.send(socket, message)
      {:noreply, %NativeBridge{state | requests: Map.put(reqs, ref, {from, message})}}
    else
      {:reply, ":ok", state}
    end
  end

  def handle_call({:register_fun, fun}, _from, state = %NativeBridge{funs: funs}) do
    ref = System.unique_integer([:positive])
    funs = Map.put(funs, ref, fun)
    {:reply, ref, %NativeBridge{state | funs: funs}}
  end

  @impl true
  def handle_info(
        {:tcp, _port, <<0::unsigned-size(64), json::binary>>},
        state = %NativeBridge{subscribers: subscribers, events: events}
      ) do
    event = decode!(json)

    if [] == subscribers do
      Logger.info("no subscriber for event #{inspect(event)}")
      {:noreply, %NativeBridge{state | events: events ++ [event]}}
    else
  ...
  def handle_info(
        ...
        state = %NativeBridge{requests: reqs}
      ) do
