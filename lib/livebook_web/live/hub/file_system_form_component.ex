defmodule LivebookWeb.Hub.FileSystemFormComponent do
  ...
  @impl true
  def render(assigns) when assigns.hub.id == "personal-hub" do
    ~H"""
      ...
      <div>
        ...
        <div class="flex gap-y-6 sm:gap-x-4">
          <.radio_card_input
            ...
            disabled={@mode == :edit}
          >
            Share a file storage that anyone can read and write.
          </.radio_card_input>
          ...
          <.radio_card_input
            ...
            disabled={@mode == :edit}
          >
            Load your templates and files directly from a Git provider that's read only.
          </.radio_card_input>
        </div>
      </div>
      <.render_form :if={@file_system} {assigns} />
    </div>
    """
  end

  defp file_system_form(%{file_system: %FileSystem.S3{}} = assigns) do
    ~H"""
  ...
  defp file_system_form(%{hub: %{id: "team-" <> _}, file_system: %FileSystem.Git{}} = assigns) do
    ~H"""
    <div class="flex flex-col space-y-4">
      <.text_field
        field={@form[:repo_url]}
      ...
      <.text_field
        ...
        placeholder="main"
      />
      <.password_field field={@form[:key]} label="Deploy Key" />
    </div>
    """
  end
