defmodule LivebookWeb.SessionLive.K8sRuntimeComponent do
  ...
  def mount(socket) do
    ...
    kubeconfig = Kubereq.Kubeconfig.load(Livebook.K8sAPI.Kubeconfig)
  ...
  def set_context(socket, context) do
    ...
    |> start_async(:load_namespace_options, fn ->
      Livebook.K8sAPI.CanI.create_access_reviews(reqs)
    end)
    |> assign(
      kubeconfig: kubeconfig,
      context: context,
  ...
  defp set_namespace(socket, ns) do
    ...
    with :ok <- CanI.can_i?(reqs, verb: "create", version: "v1", resource: "pods", namespace: ns),
         :ok <- CanI.can_i?(reqs, verb: "get", version: "v1", resource: "pods", namespace: ns),
         :ok <- CanI.can_i?(reqs, verb: "list", version: "v1", resource: "pods", namespace: ns) do
      assign(socket, namespace: ns)
    else
