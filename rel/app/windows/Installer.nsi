...
Section "Check"
  ...
  ; anyway.
  nsExec::ExecToLog '"$INSTDIR\rel\vendor\livebook-${LIVEBOOK_VERSION}-bad\otp\erts-${ERTS_VERSION}\bin\erl.exe" -noinput -eval "erlang:display(ok), halt()." 2>$R0'
  Pop $0
  ${If} $0 != 0
  ...
  nsExec::ExecToLog '"$INSTDIR\rel\vendor\livebook-${LIVEBOOK_VERSION}-bad\otp\erts-${ERTS_VERSION}\bin\erl.exe" -sname "livebook-install-test" -noinput -eval "erlang:display(ok), halt()." 2>$R0'
  Pop $0
  ${If} $0 != 0
