defmodule Livebook.ZTA.LivebookTeamsTest do
  ...
  describe "authenticate/3" do
    ...
    test "gets the user information from Livebook Teams",
      ...
      [redirect_to] = get_resp_header(conn, "location")
      assert redirect_to =~ "/oauth2/auth?hash="

      uri = URI.parse(redirect_to)
      %{"hash" => hash} = URI.decode_query(uri.query)

      %{code: code} = erpc_call(node, :allow_oauth2_request, [hash])

      conn =
        conn(:get, "/", %{auth: "true", hash: hash, code: code})
